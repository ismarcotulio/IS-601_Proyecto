...
export const tableFuncionProceduresFill = async (req, res) => {
    ...
    await pool.request()
    .query("CREATE TABLE dbo.CONTRACTS( int_contract_id_PK INTEGER IDENTITY(1,1) PRIMARY KEY, dat_hiringDate DATE NOT NULL, bit_active BIT NOT NULL, mon_salary MONEY NOT NULL, tin_position_id_FK TINYINT NOT NULL REFERENCES dbo.POSITION(tin_position_id_PK))");
    await pool.request()
    ...
    await pool.request()
    .query("CREATE TABLE dbo.EMPLOYEES( int_employee_id_PK INTEGER IDENTITY(1,1) PRIMARY KEY, var_code VARCHAR(10) NOT NULL, tin_branch_id_FK TINYINT REFERENCES dbo.BRANCH_OFFICES(tin_id_branch_PK), big_person_id_FK BIGINT NOT NULL REFERENCES dbo.PERSON(big_person_id_PK) ON DELETE CASCADE ON UPDATE CASCADE, int_contract_id_FK INTEGER NOT NULL REFERENCES dbo.CONTRACTS(int_contract_id_PK), tin_area_id_FK TINYINT NOT NULL REFERENCES dbo.AREA(tin_area_id_PK), CONSTRAINT EMPLOYEES_CODE_unique UNIQUE(var_code), CONSTRAINT EMPLOYEES_PERSON_unique UNIQUE(big_person_id_FK))");
    await pool.request()
    ...
    await pool.request()
    .query("CREATE PROCEDURE dbo.SalaryCalculate AS DECLARE @Acumuladora MONEY,@salaryE INT SET @salaryE = 1 WHILE @salaryE<=(SELECT COUNT(*) FROM dbo.SALARY) BEGIN SET @Acumuladora = (SELECT mon_salary FROM dbo.SALARY_EMP SE INNER JOIN dbo.EMPLOYEES E ON E.int_employee_id_PK=SE.int_employee_id_FK INNER JOIN dbo.CONTRACTS C ON C.int_contract_id_PK=E.int_contract_id_FK WHERE SE.int_salary_id_FK=@salaryE) UPDATE dbo.SALARY SET mon_hourSalary = @Acumuladora/160 WHERE SALARY.int_salary_id_PK=@salaryE SET @Acumuladora = @Acumuladora + ISNULL((SELECT SUM(EX.tin_amount*SA.mon_hourSalary*(TY.flo_porcent/100)) FROM dbo.EXTRA_HOURS EX,dbo.SALARY SA,dbo.TYPE_HOURS TY WHERE EX.bit_payFactor=1 AND SA.int_salary_id_PK=@salaryE AND EX.int_salary_id_FK=@salaryE AND EX.tin_hourType_id_FK=TY.tin_hourType_id_PK AND EX.int_salary_id_FK=SA.int_salary_id_PK),0) SET @Acumuladora = @Acumuladora + ISNULL((SELECT SUM(MO.int_factor*MO.mon_amount) FROM dbo.SALARY SA,dbo.PAYMENT_MOVEMENT PM,dbo.MOVEMENT MO WHERE PM.int_salary_id_PK_FK=SA.int_salary_id_PK AND PM.int_movement_id_PK_FK = MO.int_movement_id_PK AND PM.bit_motionFactor=1 AND SA.int_salary_id_PK=@salaryE AND PM.int_salary_id_PK_FK=@salaryE),0) UPDATE dbo.SALARY SET mon_netSalary = @Acumuladora WHERE SALARY.int_salary_id_PK=@salaryE print @Acumuladora SET @salaryE = @salaryE + 1 END")
    await pool.request()
    .query("CREATE PROCEDURE dbo.CreateSalaryEmployees AS DECLARE @dateEmployee DATE, @employee INT,@dateYear DATE, @ACUM INT,@ACUMM INT; SET NOCOUNT OFF; SET @ACUM = 1; SET @employee = 1; SET @ACUMM = (SELECT COUNT(*) FROM dbo.CONTRACTS); WHILE(@ACUMM>=@employee) BEGIN SET @dateEmployee = (SELECT dat_hiringDate FROM dbo.CONTRACTS WHERE int_contract_id_PK=@employee); SET @dateYear = DATEADD(DAY,30,@dateEmployee); WHILE(@dateYear<='2021-12-31') BEGIN INSERT INTO dbo.SALARY(mon_netSalary,mon_hourSalary,dat_date,tin_area_id_FK) VALUES (1,0,@dateYear,(SELECT tin_area_id_FK FROM dbo.EMPLOYEES WHERE int_employee_id_PK=@employee)); INSERT INTO dbo.SALARY_EMP(bit_pay,int_salary_id_FK,int_employee_id_FK) VALUES (1,@ACUM,@employee); SET @dateYear = DATEADD(DAY,30,@dateYear); SET @ACUM = @ACUM+1; END print @dateEmployee; SET @employee = @employee+1; END")
    await pool.request()
