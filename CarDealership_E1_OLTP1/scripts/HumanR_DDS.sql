...
CREATE TABLE CONTRACTS(
  ...
  tin_position_id_FK TINYINT NOT NULL REFERENCES POSITION(tin_position_id_PK)
);
...
CREATE TABLE dbo.TYPE_REGISTRATION(
  tin_typeRegistration_id_PK TINYINT PRIMARY KEY,
  var_name VARCHAR(50) NOT NULL,
  tex_description TEXT NOT NULL,
);

CREATE TABLE dbo.REGISTRATION_HIRES(
  int_registration_id_PK INT PRIMARY KEY,
  int_contract_id_FK INTEGER NOT NULL REFERENCES CONTRACTS(int_contract_id_PK),
  dat_hiringDate DATE NOT NULL,
  int_employee_id_FK INTEGER NOT NULL REFERENCES EMPLOYEES(int_employee_id_PK),
  tin_typeRegistration_id_FK TINYINT NOT NULL REFERENCES TYPE_REGISTRATION(tin_typeRegistration_id_PK),
);
